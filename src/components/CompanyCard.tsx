...
export default function CompanyCard({ company }: { company: Company }) {
  return (
    <div className="group relative rounded-2xl border-2 bg-white shadow-sm overflow-hidden transition-shadow hover:shadow-md" style={{ borderColor: '#5D9252' }}>
      ...
      <div className="p-4">
        <div className="text-lg sm:text-xl font-semibold text-gray-800 truncate">{company.name}</div>
        <Link href={`/company/${company.id}`} className="mt-2 inline-block text-xs text-emerald-700 hover:underline">
          View more →
        </Link>
      </div>
    </div>
